...
namespace Cotador
{
	class Extenso
	{
		public static string EscreverExtenso(string alor)
		{
			if (alor.Length == 0) return "";
			char[] az = Enumerable.Range('a', 'z' - 'a' + 1).Select(i => (Char)i).ToArray();

			foreach (char l in az)
			{
				if (alor.ToLower().Contains(l))
					return alor;
			}
			string letra = alor;
			//letra = az.ToString("N", new CultureInfo("en-us", false));
